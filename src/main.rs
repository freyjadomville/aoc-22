fn main() {
    ...
    let mut elf_totals = input.lines().fold(vec![], |mut elves:Vec<u32>, new_value| {
        if elves.len() == 0 { elves.push(new_value.parse().expect("First value wasn't a u32"));}
        else if new_value == "" { elves.push(0) }
        else { 
